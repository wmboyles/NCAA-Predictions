        ...
        with open("./games/"+str(year)+"/"+dashed_team_name+"_games.csv", 'r', newline='') as in_csv:
            ...
            for row in reader:
                ...
                try:
                    ...
                    if A_FGA == 0: A_eFGp = 0
                    else: A_eFGp = (A_FG + .5*A_3P)/A_FGA
                    if B_FGA == 0: B_eFGp = 0
                    else: B_eFGp = (B_FG + .5*B_3P)/B_FGA

                    if A_FGA == 0 and A_FTA == 0 and A_TOV == 0: A_TOVp = 0
                    else: A_TOVp = A_TOV/(A_FGA + .44*A_FTA + A_TOV)
                    if B_FGA == 0 and B_FTA == 0 and B_TOV == 0: B_TOVp = 0
                    else:  B_TOVp = B_TOV/(B_FGA + .44*B_FTA + B_TOV)

                    if A_ORB == 0 and B_ORB == 0: A_ORBp, B_ORBp = 0, 0
                    else: A_ORBp, B_ORBp = A_ORB/(A_ORB + B_ORB), B_ORB/(A_ORB + B_ORB)
                    
                    if A_FTA == 0: A_FTR = 0
                    else: A_FTR = A_FT/A_FTA
                    if B_FTA == 0: B_FTR = 0
                    else: B_FTR = B_FT/B_FTA
                

                    new_row = [A, B, WL, A_score, B_score, A_eFGp, B_eFGp, A_TOVp, B_TOVp, A_ORBp, B_ORBp, A_FTR, B_FTR]
                    new_rows.append(new_row)
                except ValueError:
                    print("---WARNING:", dashed_team_name, opponent,year, "misisng values")
                    continue
